...
print(f"ADK tool '{adk_tool_to_expose.name}' initialized and ready to be exposed via MCP.", file=sys.stderr)
# --- End ADK Tool Prep ---
...
async def list_mcp_tools() -> list[mcp_types.Tool]:
    ...
    print(f"MCP Server: Advertising tool: {mcp_tool_schema.name}\n\tmcp_tool_schema: {mcp_tool_schema}", file=sys.stderr)
...
async def call_mcp_tool(
    ...
    if name == adk_tool_to_expose.name:
        try:
            ...
            print(f"MCP Server: ADK tool '{name}' executed. Response: {adk_tool_response}", file=sys.stderr)
        ...
        print(f"MCP Server: Tool '{name}' not found/exposed by this server.", file=sys.stderr)
...
async def run_mcp_stdio_server():
    ...
    async with mcp.server.stdio.stdio_server() as (read_stream, write_stream):
        print("MCP Stdio Server: Starting handshake with client...", file=sys.stderr)
        await app.run(
            read_stream,
            write_stream,
        ...
        print("MCP Stdio Server: Run loop finished or client disconnected.", file=sys.stderr)

if __name__ == "__main__":
    print("Launching MCP Server to expose ADK tools via stdio...", file=sys.stderr)
    try:
        asyncio.run(run_mcp_stdio_server())
    except KeyboardInterrupt:
        print("\nMCP Server (stdio) stopped by user.", file=sys.stderr)
    except Exception as e:
        print(f"MCP Server (stdio) encountered an error: {e}", file=sys.stderr)
    finally:
